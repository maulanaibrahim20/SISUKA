...
@section('content')
    <div class="container-xxl flex-grow-1 container-p-y">
        <div
            ...
            <h5 class="card-title mb-sm-0 me-2"><span class="text-muted fw-light">Create Account User /</span>
                {{ $page_active }}
            </h5>
            <div class="action-btns">
                <a href="{{ url('/admin/create/owner/create') }}" class="btn btn-primary fa fa-plus"> Add New
                    Owner</a>
            </div>
        </div>
                        ...
                        @forelse ($user as $data)
                            <tr>
                                ...
                                <td class="text-center">
                                    <a href="{{ url('/admin/create/owner/' . $data->id_owner . '/edit') }}"
                                        class="btn btn-warning"><i class="ti ti-edit"></i></a>
                                    <button type="button" class="btn btn-primary" data-bs-toggle="modal"
                                        data-bs-target="#modalCenter{{ $data->id_owner }}">
                                        <i class="ti ti-eye"></i></button>
                                    <form id="deleteForm{{ $data->id_owner }}"
                                        action="{{ url('/admin/create/owner/' . $data->id_owner) }}"
                                        style="display: inline;" method="POST">
                                        @method('DELETE')
                                        @csrf
                                        <button type="button" class="btn btn-danger deleteBtn"
                                            data-id="{{ $data->id_owner }}"><i class="ti ti-trash"></i></button>
                                    </form>
                                </td>
                            </tr>
                        @empty
